...
def forecast_demand(df_sales, forecast_model, forecast_days, forecast_window_size):
    """
    ...
    for sku in unique_skus:
        for location in unique_locations:
            ...
            if forecast_model == "Moving Average":
                window_size = forecast_window_size
...
def run_full_simulation(
    ...
    if forecast_days > 0:
        ...
        df_forecast = forecast_demand(df_sales, forecast_model, forecast_days, forecast_window_size)
    else:
...
if 'df_sales_for_plot' not in st.session_state:
...
# simulation_days = st.sidebar.slider("Simulation Duration (days)", 30, 365, 90) # Removed as per user request
...
forecast_horizon_days = st.sidebar.slider("Forecast Horizon (days)", 1, 365, 90)

forecast_window_size = 7
if forecast_model in ["Moving Average", "Moving Median"]:
    forecast_window_size = st.sidebar.slider("Forecast Window Size (Days)", 1, 30, 7)
...
if run_simulation_button:
    ...
    with st.spinner("Preparing and running simulation... This may take a moment."):
        ...
        # Store all_dfs in session state
        ...
        if not all(all_dfs.get(f) is not None and not all_dfs.get(f).empty for f in required_files):
            ...
            # Store df_sales for plotting the forecast line
            st.session_state.df_sales_for_plot = df_sales.copy()
...
# Display results if simulation has been run (data is in session_state)
if st.session_state.simulation_results is not None:
    simulation_results = st.session_state.simulation_results
    df_sales_for_plot = st.session_state.df_sales_for_plot
    ...
    selected_location = st.selectbox("Select Location", all_locations, key="location_selector")
    selected_item = st.selectbox("Select SKU/Component", all_items, key="item_selector")
    ...
    if not df_plot.empty:
        ...
        if not df_sales_for_plot.empty:
            forecast_start_date_plot = df_sales_for_plot['Date'].max()
            fig.add_vline(x=str(forecast_start_date_plot.date()), line_dash="dash", line_color="red", annotation_text="Forecast Start", annotation_position="top right")
