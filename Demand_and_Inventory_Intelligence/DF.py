...
with st.expander("What does the BOM check do?"):
    st.markdown("""
        ...
        A factory will only be able to fulfill an order if it has all the necessary components for that SKU in its `component_inventory`. If a component is missing, the order cannot be fulfilled, which can can lead to a delayed shipment and a potential stockout at the downstream location. This accurately reflects real-world production constraints.
    """)
